...
const NearbyVeterinarians = () => {
  ...
  const handleCall = (vet) => {
    void trackVetInteraction(vet.id, 'call_click', 'nearby-veterinarians');
    window.open(`tel:${vet.phone_number}`, '_self');
  };

  const handleWhatsApp = (vet) => {
    void trackVetInteraction(vet.id, 'whatsapp_click', 'nearby-veterinarians');
