...
const CowListingForm = ({ navigation, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('📤 [COW LISTING] Submitting to: /api/animals/listings');
      
      // Debug: Check if user is authenticated
      const storedToken = await AsyncStorage.getItem('token');
      console.log('🔑 [COW LISTING] Token exists:', !!storedToken);
      console.log('👤 [COW LISTING] User:', user ? 'Logged in' : 'Not logged in');
      ...
      console.log('✅ [COW LISTING] Success:', response.data);
      ...
      console.error('❌ [COW LISTING] Full Error:', error);
      console.error('❌ [COW LISTING] Error Response:', error.response?.data);
      console.error('❌ [COW LISTING] Error Status:', error.response?.status);
      console.error('❌ [COW LISTING] Error Message:', error.message);
      console.error('❌ [COW LISTING] Error Code:', error.code);
    ...
    >
      ...
      <View style={styles.section}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            style={styles.input}
            placeholder={t('animal.agePlaceholder') || 'e.g., 3 years, 2nd lactation'}
            value={formData.age}
